...
class VILBertForVLTasks(BertPreTrainedModel):
    def __init__(self, config, dropout_prob=0.1, default_gpu=True):
        ...
        self.vil_prediction = nn.Linear(config.v_hidden_size, 1)
        #         self.max_object_classes = 1001
        #         self.vil_prediction = SimpleClassifier(config.bi_hidden_size, config.bi_hidden_size * 2, self.max_object_classes, 0.5)
        ...
        self.fusion_method = config.fusion_method
        self.entropy_classifier = nn.Sequential(nn.LayerNorm(1), nn.Linear(1, 32), nn.ReLU(), nn.Linear(32, 1))
        self.sim_classifier = nn.Sequential(nn.LayerNorm(1), nn.Linear(1, 1))
        self.ambiguity_classifier = AmbiguityClassifier(input_dim=5)
        self.object_sim_classifier = nn.Sequential(nn.LayerNorm(1), nn.Linear(1, 32), nn.ReLU(), nn.Linear(32, 1))
        self.ambiguous_prediction_head = nn.Sequential(
            nn.Linear(config.bi_hidden_size, 128),
            nn.ReLU(),
            nn.Linear(128, 1)
        )
    ...
    def forward(
        ...
        text_emb = pooled_output_t
        image_emb = pooled_output_v
        vil_prediction = self.vil_prediction(self.dropout(sequence_output_v)).squeeze(-1)
        ...
        ambiguous_logit = self.ambiguous_prediction_head(pooled_output)
        ambiguous_logit = ambiguous_logit.squeeze(1)
